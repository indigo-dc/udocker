...
class RuncEngine(ExecutionEngineCommon):
    ...
    def select_runc(self):
        ...
        if not os.path.exists(self.executable):
            LOG.error("runc or crun executable not found")
    ...
    def _add_device_spec(self, dev_path, mode="rwm"):
        ...
        if not (os.path.exists(dev_path) and dev_path.startswith("/dev/")):
            LOG.error("device not found: %s", dev_path)
            return False
        ...
        if stat.S_ISBLK(dev_stat.st_mode):
            ...
            LOG.error("not a device: %s", dev_path)
            return False
    ...
    def run_nopty(self, cmd_l):
        ...
        status = subprocess.Popen(cmd_l, shell=False, close_fds=False, stdout=pslave, stderr=pslave)
        os.close(pslave)
        while True:
            status.poll()
