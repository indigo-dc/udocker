...
class UdockerToolsTestCase(TestCase):
    ...
    def test_09__verify_version(self, mock_isfile, mock_fumktmp,
        ...
        mock_fugetdata.return_value = "1.2.5"
        mock_furm.return_value = None
        ...
        with patch.object(tarfile, 'open', autospec=True) as open_mock:
            ...
            utools = UdockerTools(self.local)
            status = utools._verify_version(tball)
            self.assertEqual(status, (True, "1.2.5"))
            self.assertTrue(mock_furm.called)
    ...
    def test_11__get_mirrors(self):
        ...
        mirrors = "https://download.ncg.ingrid.pt/udocker-1.2.5.tar.gz"
        utools = UdockerTools(self.local)
    ...
    def test_12_get_installinfo(self, mock_jload, mock_mirr, mock_getf):
        ...
        res = {"tarversion": "1.2.5"}
        mock_jload.return_value = {"tarversion": "1.2.5"}
        mock_mirr.return_value = ["/home/info.json"]
        mock_getf.return_value = "info.json"
        subuid_line = StringIO('{"tarversion": "1.2.5"}')
    ...
    def test_13__install_logic(self, mock_furm, mock_getmirr, mock_getfile,
        ...
        mock_furm.return_value = None
        mock_getmirr.return_value = "https://down.pt/udocker-1.2.5.tar.gz"
        mock_getfile.return_value = "udocker-1.2.5.tar.gz"
        mock_verversion.return_value = (True, "1.2.5")
    ...
    def test_14_install(self, mock_isavail, mock_instlog, mock_getinfo,
        ...
        Config.conf['tarball'] = "udocker-1.2.5.tar.gz"
        Config.conf['tarball_release'] = "1.2.5"
